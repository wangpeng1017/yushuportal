...
service.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    ...
    config.headers['Accept-Language'] = useLocaleStore().currentLocale.lang || 'zh-CN'
...
service.interceptors.response.use(
  async (response: AxiosResponse<any>) => {
    ...
    // 如果响应中明确标记为成功（success: true），即使 code 不是 200 也视为成功
    if (data.success === true) {
      return data
    }
    // 获取错误信息
    let msg = data.message || errorCode[code] || errorCode['default']
    // 尝试翻译错误消息（如果是 i18n key，如 outbound.common.saveFailed）
    if (msg && typeof msg === 'string' && msg.includes('.')) {
      try {
        const translatedMsg = i18n?.global?.t(msg)
        // 如果翻译成功（返回的不是原 key），使用翻译后的消息
        if (translatedMsg && translatedMsg !== msg) {
          msg = translatedMsg
        }
      } catch (e) {
        // 翻译失败，使用原消息
    ...
    if (ignoreMsgs.indexOf(msg) !== -1) {
      ...
      ElMessage.error({
        ...
        message:
          ...
          '</div>' +
          '<div> &nbsp; </div>' +
          '<div>参考 https://iimake.com/ 教程</div>' +
          '<div> &nbsp; </div>' +
          '<div>5 分钟搭建本地环境</div>'
      })
      return Promise.reject(new Error(msg))

  ...
  <div class="spot-inspection-plan-page">
    ...
    >
      ...
      >
        ...
        <el-table-column label="循环周期" align="center" prop="periodicFrequencyType" width="120" />
        ...
        <el-table-column label="是否可跳过" align="center" prop="skipCode" width="100">
          <template #default="scope">
            {{ eamEnumStore.getYesNoText(scope.row.skipCode) }}
          </template>
        </el-table-column>
    ...
    <!-- ==================== 设备清单 ==================== -->
    <ContentWrap>
      <div class="item-section-title">设备清单</div>
    ...
    <ContentWrap v-if="!isParamPlan">
      ...
      <div class="item-section-toolbar">
        ...
        >
          <Icon icon="ep:document-add" class="mr-5px" />&nbsp;选择标准
        </el-button>
        <el-button
          v-hasPermi="[PERMI.CREATE]"
          plain
          type="primary"
          :disabled="!currentPlan"
          @click="openItemForm('create', '定性')"
        >
          <Icon icon="ep:plus" class="mr-5px" />&nbsp;新增定性
        </el-button>
        <el-button
          v-hasPermi="[PERMI.CREATE]"
          plain
          type="primary"
          :disabled="!currentPlan"
          @click="openItemForm('create', '定量')"
        >
          <Icon icon="ep:plus" class="mr-5px" />&nbsp;新增定量
        </el-button>
        <el-button
          v-hasPermi="[PERMI.DELETE]"
          plain
      ...
      >
        ...
        <el-table-column label="正常项" align="center" prop="normal" min-width="200">
          <template #default="scope">
            <div v-if="scope.row.itemType === '定性' && scope.row.normal" class="tag-cell">
              <el-tag
        ...
        <el-table-column label="异常项" align="center" prop="abnormal" min-width="200">
          <template #default="scope">
            <div v-if="scope.row.itemType === '定性' && scope.row.abnormal" class="tag-cell">
              <el-tag
        ...
        <el-table-column label="上限值" align="center" prop="max" width="80">
          <template #default="scope">{{
            scope.row.itemType === '定量' ? scope.row.max || '--' : '--'
          }}</template>
        </el-table-column>
        <el-table-column label="下限值" align="center" prop="min" width="80">
          <template #default="scope">{{
            scope.row.itemType === '定量' ? scope.row.min || '--' : '--'
          }}</template>
        </el-table-column>
        ...
        <el-table-column label="单位" align="center" prop="unitName" width="80">
          <template #default="scope">{{
            scope.row.itemType === '定量' ? scope.row.unitName || '--' : '--'
          }}</template>
        </el-table-column>
        <el-table-column label="必检项" align="center" prop="needTypeText" width="80" />
        <el-table-column label="检查标准" align="center" prop="checkStandard" width="150">
          <template #default="scope">{{ scope.row.checkStandard || '--' }}</template>
        </el-table-column>
        ...
        <el-table-column label="操作" align="center" fixed="right" width="140">
          <template #default="scope">
            <el-button
              ...
              class="btn-edit"
              v-hasPermi="[PERMI.UPDATE]"
              @click="openItemForm('edit', undefined, scope.row)"
            >
              &nbsp;编辑
            </el-button>
            <el-button
    ...
    <!-- 参数巡检提示 -->
    <ContentWrap v-else-if="currentPlan">
      <el-alert
        title="当前为参数巡检计划，检查项来自参数巡检配置模块"
        type="info"
        :closable="false"
        show-icon
      />
    ...
    <!-- 标准选择弹窗（单选，复用 TableSelectDialog） -->
    <TableSelectDialog
...
defineOptions({ name: 'EamSpotInspectionPlan' })
...
// ==================== 权限标识 ====================
const PERMI = {
  QUERY: 'eam:spotInspectionPlan:query',
  CREATE: 'eam:spotInspectionPlan:create',
  UPDATE: 'eam:spotInspectionPlan:update',
  DELETE: 'eam:spotInspectionPlan:delete'
}
...
/** 当前选中计划是否为参数巡检 */
const isParamPlan = computed(() => currentPlan.value?.hasParamPlan === '1')
...
const deviceList = ref<PlanApi.PlanDeviceVo[]>([])
...
const getDeviceList = async () => {
  ...
  try {
    const res = await PlanApi.getDevicePage({
...
const handleDeviceSelectionChange = (rows: PlanApi.PlanDeviceVo[]) => {
  deviceSelectedIds.value = rows.map((r) => r.id)
}

const handleDeviceDelete = async (row: PlanApi.PlanDeviceVo) => {
  try {
    await message.delConfirm()
    await PlanApi.deleteDevice(row.id)
    message.success('删除成功')
    deviceSelectedIds.value = []
    getDeviceList()
...
const handleDeviceBatchDelete = async () => {
  try {
    await message.delConfirm()
    await PlanApi.batchDeleteDevice(deviceSelectedIds.value.join(','))
    message.success('批量删除成功')
    deviceSelectedIds.value = []
    getDeviceList()
    getList() // 刷新主表 deviceNum
...
const handleDevSelectorConfirm = async () => {
  ...
  const devices: PlanApi.PlanDeviceSaveDto[] = devSelectorSelected.value.map((row) => ({
    planCode: currentPlan.value!.code,
  ...
  try {
    await PlanApi.addDevices(devices)
...
const standardColumns: TableColumn[] = [
  ...
  { prop: 'equipmentTypeText', label: '设备类型', width: 150 },
  { prop: 'checknum', label: '检查项数', width: 100 }
]
...
const openItemForm = (mode: 'create' | 'edit', presetType?: string, row?: PlanApi.PlanItemVo) => {
  itemFormRef.value.open(mode, currentPlan.value!, presetType, row)
}
...
// ==================== 初始化 ====================
onMounted(async () => {
  await eamEnumStore.loadSpotInspectionPlanEnums()
  await getList()
})
</script>

<style lang="scss" scoped>
.spot-inspection-plan-page {
  height: 100%;
}

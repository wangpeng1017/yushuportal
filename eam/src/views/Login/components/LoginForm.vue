      ...
      <el-col :span="24" class="px-10px">
        <el-form-item>
          <XButton
            ...
            :title="t('login.login')"
            class="w-full login-btn-primary"
            type="success"
            @click="getCode()"
          />
        </el-form-item>
...
const ssoLogin = async () => {
  ...
  window.location.href = `${import.meta.env.VITE_IIMAKE_SSO_URL}?client_id=${import.meta.env.VITE_IIMAKE_CLIENT_ID}&response_type=code&auto_approve=true&scope=%7B%22user.read%22%3Atrue%7D&redirect_uri=${encodeURIComponent(redirect)}`
}

const codeLogin = async () => {
  // SSO 回调地址缓存
  const redirect_new: string = `${window.location.origin}/login?redirect=/index`
  localStorage.setItem('redirect_uri', redirect_new)
  const queryParams = currentRoute.value.query
  const code = queryParams.code
  if (code) {
    const redirect_uri =
      localStorage.getItem('redirect_uri') || window.location.origin + window.location.pathname
    const loginResult = await LoginApi.codeLogin({
      code: code,
      url: redirect_uri?.replaceAll('#', '%23')
    })
    if (loginResult && loginResult.username) {
      ...
      // authUtil.setToken(auth) 需要把Token放到浏览器中
      authUtil.setTenantId(1) // loginResult.tenantId
      loginLoading.value = true
      try {
        const loginForm = {

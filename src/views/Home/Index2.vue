                ...
                <el-row :gutter="16" class="project-grid">
                  ...
                  >
                    <div
                      ...
                      @click="handleProjectClick(item.redirectUris[0], item.clientId)"
                    >
...
const getProject = async () => {
  ...
  try {
    ...
    const filteredData = Array.isArray(data)
      ? data.filter((item) => item.id !== '1' && item.id !== 1)
      : data
...
const handleProjectClick = (message: string, uuid: string) => {
  // 判断是否为外部链接(以http://或https://开头)
  if (message.startsWith('http://') || message.startsWith('https://')) {
    // 外部链接,在新窗口打开
    window.open(message, '_' + uuid)
  } else {
    // 内部路由,使用router跳转
    router.push(message)
  }

<template>
  <doc-alert title="应用管理" url="https://iimake.com/app/" />
  ...
  <ContentWrap>
    ...
    <el-table v-loading="loading" :data="list" @selection-change="handleRowCheckboxChange">
      ...
      <el-table-column label="应用编码" width="120" align="center" prop="code" />
      <el-table-column label="客户端" width="120" align="center" prop="clientId">
        <template #default="scope">
          {{ getClientNameById(scope.row.clientId) }}
        </template>
      </el-table-column>
      ...
      <el-table-column label="应用描述" width="160" align="center" prop="description" />
  ...
  <!-- 表单弹窗：绑定菜单 -->
  <AppAssignMenuForm ref="assignMenuFormRef" @success="getList" />
</template>
...
import AppAssignMenuForm from './assignMenuForm.vue'
...
const clientMap = ref<Record<number, string>>({}) // 客户端映射表
...
/** 查询客户端列表并建立映射 */
const loadClientMap = async () => {
  try {
    const response = await ClientApi.getOAuth2ClientList({})
    const clients = response || []
    clientMap.value = {}
    clients.forEach((client: any) => {
      clientMap.value[client.id] = client.name
    })
  } catch (error) {
...
const getClientNameById = (clientId: number) => {
  return clientMap.value[clientId] || '未知客户端'
}
...
const openAssignMenuForm = async (row: AppApi.AppVO) => {
...
const handleRowCheckboxChange = (rows: AppApi.AppVO[]) => {
...
const handleDeleteBatch = async () => {
  try {
    ...
    await message.delConfirm()
    // 发起批量删除
    await AppApi.deleteAppList(checkedIds.value)
    checkedIds.value = []
    message.success(t('common.delSuccess'))
    // 刷新列表
    await getList()

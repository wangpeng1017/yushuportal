  ...
  <Dialog v-model="dialogVisible" :title="`菜单权限，角色名称：${formData.name}，角色标识：${formData.code}`" width="900px">
    <el-form ref="formRef" v-loading="formLoading" :model="formData" label-width="100px">
      <el-form-item label="应用选择">
        <el-tabs v-model="activeClientId" type="card" class="w-full" @tab-click="handleClientChange">
          <el-tab-pane
...
const clientList = ref<any[]>([]) // 应用列表
const activeClientId = ref<string>('') // 当前激活的应用ID
...
const open = async (row: RoleApi.RoleVO) => {
  ...
  try {
    // 加载应用列表
    const clients = await ClientApi.getOAuth2ClientList({
      status: 0,
      deleted: 0,
      internal: true
    } as any)
    ...
    if (clientList.value.length > 0) {
      activeClientId.value = String(clientList.value[0].id)
      // 加载第一个应用的菜单和权限
      await loadClientMenus(clientList.value[0].id)
    }
  } finally {
...
/** 加载指定应用的菜单和权限 */
const loadClientMenus = async (clientId: number) => {
  if (!formData.id) return
  
  formLoading.value = true
  try {
    // 加载 Menu 列表
    const menus = await MenuApi.getClientMenusList(clientId)
    ...
    // 加载角色在该应用下已分配的菜单权限
    const menuIds = await PermissionApi.getRoleClientMenuList(formData.id, clientId)
...
/** 处理应用切换 */
const handleClientChange = async (pane: any) => {
  const clientId = pane.paneName || pane.props?.name
  if (clientId) {
    await loadClientMenus(Number(clientId))
  }
}
...
const saveCurrentClient = async () => {
  ...
  try {
    ...
    const data = {
      roleId: formData.id,
      clientId: Number(activeClientId.value),

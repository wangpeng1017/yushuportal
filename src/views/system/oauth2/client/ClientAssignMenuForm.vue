...
defineOptions({ name: 'ClientAssignMenuForm' })
...
/** 打开弹窗 */
const open = async (row: ClientApi.OAuth2ClientVO) => {
  ...
  try {
    formData.menuIds = await ClientApi.getClientMenuList(row.id)
    // 设置选中
...
const submitForm = async () => {
  ...
  try {
    ...
    await ClientApi.assignClientMenu(data)

...
export interface Oauth2AssignClientMenuReqVO {
  clientId: number
  menuIds: number[]
}
...
// 查询 OAuth2 客户端的列表
export const getOAuth2ClientList = (params: PageParam) => {
  return request.get({ url: '/system/oauth2-client/list', params })
}
...
// 查询角色拥有的菜单权限
export const getClientMenuList = async (clientId: number) => {
  return await request.get({ url: '/system/oauth2-client/menu/list-client-menus?clientId=' + clientId })
}

// 赋予角色菜单权限
export const assignClientMenu = async (data: Oauth2AssignClientMenuReqVO) => {
  return await request.post({ url: '/system/oauth2-client/menu/assign-client-menu', data })
}
